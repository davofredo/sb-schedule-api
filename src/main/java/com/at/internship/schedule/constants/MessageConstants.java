package com.at.internship.schedule.constants;

public class MessageConstants {
    public final static String STR_CODE_OK = "OK";
    public final static String STR_MESSAGE_SUCCESS = "Success!";
    public final static String STR_CODE_VALIDATION_FAILED = "VALIDATION_FAILED";
    public final static String STR_MESSAGE_VALIDATION_FAILED = "There were validation errors";
    public final static String STR_CODE_RECORD_NOT_FOUND= "RECORD_NOT_FOUND";
    public final static String STR_MESSAGE_RECORD_NOT_FOUND = "Record not found";
    public final static String STR_CODE_DB_CONSTRAINT_VIOLATION= "DB_CONSTRAINT_VIOLATION";
    public final static String STR_MESSAGE_DB_CONSTRAINT_VIOLATION= "Database constraint violation";

}
